...
async function updatePriorityHelpText(): Promise<void> {
  try {
    ...
    for (const template of templates) {
      ...
      try {
        if (typeof template.fields === 'string') {
          parsedFields = JSON.parse(template.fields) as FormField[];
        } else if (Array.isArray(template.fields)) {
          parsedFields = template.fields as unknown as FormField[];
        } else {
      ...
      if (Array.isArray(parsedFields)) {
        updatedFields = parsedFields.map((field: FormField) => {
