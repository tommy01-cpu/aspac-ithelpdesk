...
import { Plus, Search, Filter, Download, Upload, Edit2, Trash2, Eye, ChevronLeft, ChevronRight, Save } from 'lucide-react';
...
export const SLAServiceComponent = () => {
  ...
  const mockSLAs: ServiceSLA[] = [
    ...
    },
    {
      ...
      description: 'SLA for high priority requests',
      priority: 'high',
      responseTime: 4,
      resolutionTime: 24,
    ...
    },
    {
      ...
      description: 'Standard SLA for regular service requests',
      priority: 'medium',
      responseTime: 8,
      resolutionTime: 72,
      operationalHours: true,
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      setPagination({
        ...
        pages: Math.ceil(mockSLAs.length / 25)
      });
      setLoading(false);
    }, 500);
  }, []);
  ...
  const handleDeleteSelected = async () => {
    if (selectedSLAs.length > 0 && confirm(`Are you sure you want to delete ${selectedSLAs.length} selected SLAs?`)) {
  ...
  const TimeInput = ({ 
    ...
    label: string; 
    value: { days: string; hours: string; minutes: string };
    onChange: (value: { days: string; hours: string; minutes: string }) => void;
  }) => (
    <div>
      <Label>{label}</Label>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Service SLA Management</h1>
          <p className="text-slate-600 mt-1">Manage service level agreements for your organization</p>
        </div>
        ...
        <table className="w-full">
          ...
          <tbody>
            {paginatedSLAs.map((sla) => (
              <tr key={sla.id} className="border-b border-slate-100 hover:bg-slate-50">
                <td className="p-4">
                  <Checkbox
                    checked={selectedSLAs.includes(sla.id)}
                    onCheckedChange={(checked) => {
                      if (checked) {
                        setSelectedSLAs([...selectedSLAs, sla.id]);
                      } else {
                        setSelectedSLAs(selectedSLAs.filter(id => id !== sla.id));
                      }
                    }}
                  />
                </td>
                ...
                <td className="p-4">
                  <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                    sla.priority === 'critical' ? 'bg-red-100 text-red-800' :
                    sla.priority === 'high' ? 'bg-orange-100 text-orange-800' :
                    sla.priority === 'medium' ? 'bg-blue-100 text-blue-800' :
                ...
                <td className="p-4 text-slate-600">
                  {sla.responseTime < 60 ? `${sla.responseTime} min` :
                   sla.responseTime < 1440 ? `${Math.floor(sla.responseTime / 60)} hr` :
                   `${Math.floor(sla.responseTime / 1440)} day`}
                </td>
                <td className="p-4 text-slate-600">
                  {sla.resolutionTime < 60 ? `${sla.resolutionTime} min` :
                   sla.resolutionTime < 1440 ? `${Math.floor(sla.resolutionTime / 60)} hr` :
                   `${Math.floor(sla.resolutionTime / 1440)} day`}
                </td>
                <td className="p-4">
                  <span className={`px-2 py-1 text-xs font-medium rounded-full ${
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-4">
              <TimeInput 
                ...
                onChange={(value) => setNewSLA(prev => ({ ...prev, responseTime: value }))}
              />
              
              <TimeInput 
                label="Resolution Time"
                value={newSLA.resolutionTime}
                onChange={(value) => setNewSLA(prev => ({ ...prev, resolutionTime: value }))}
              />

              <TimeInput 
                label="Escalation Time"
                value={newSLA.escalationTime}
                onChange={(value) => setNewSLA(prev => ({ ...prev, escalationTime: value }))}
              />
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="autoEscalate">Enable auto escalation</Label>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setNewSLA(prev => ({ ...prev, autoEscalate: checked as boolean }))}
                />
                <Label htmlFor="editAutoEscalate">Enable auto escalation</Label>
              </div>
            </div>
