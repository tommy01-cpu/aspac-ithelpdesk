...
const EditableNameSelect: React.FC<{
  ...
  return (
    <div className="relative">
      ...
      {/* Clear button - shows when focused and has value */}
      {showDropdown && searchInput && !disabled && (
        <button
          type="button"
          onMouseDown={(e) => {
            ...
            setSearchInput('');
            onChange('');
            // Keep dropdown open by maintaining focus
            setTimeout(() => {
              const input = buttonElement.parentElement?.querySelector('input') as HTMLInputElement;
              if (input) {
                input.focus();
              }
            }, 0);
...
export default function RequestPage() {
  ...
  // DISABLED: Auto-selection removed to allow manual technician selection
  /*
  useEffect(() => {
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-blue-50">
        ...
        <div className="container mx-auto p-3 sm:p-6 max-w-4xl">
          ...
          {/* Template Info Alert - Display for Service templates with SLA or Incident templates with priority-based SLA */}
              {(templateData?.slaService || slaData) && (
                <Alert className="mb-6 border-amber-200 bg-amber-50">
                  ...
                  <AlertDescription className="flex items-center justify-between text-amber-900">
                    <div>
                      ...
                      {templateData.type === 'incident' && slaData && (
                        <>
                          <strong> SLA:</strong> {slaData.name}
                          (Resolution: {formatDurationFromHours(
                            (slaData.resolutionDays || 0) * 24 + 
                            (slaData.resolutionHours || 0) + 
                            (slaData.resolutionMinutes || 0) / 60
                          )})
                        </>
                      )}
