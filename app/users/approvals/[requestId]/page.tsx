...
export default function ApprovalDetailsPage() {
  ...
  const getApprovalStatusDisplay = () => {
    if (!selectedApproval) return { text: 'Pending Approval', color: getApprovalStatusColor('pending approval') };
    
    switch (selectedApproval.status.toLowerCase()) {
      case 'pending clarification':
      case 'pending-clarification':
        return { text: 'Pending Clarification', color: getApprovalStatusColor('pending clarification') };
      case 'approved':
                  ...
                  <Card>
                    <CardContent className="pt-6">
                      <div className="flex gap-4">
                        ...
                        {/* Only show Need Clarification button if not already for clarification */}
                        {selectedApproval && 
                         selectedApproval.status !== 'pending clarification' && 
                         selectedApproval.status !== 'for-clarification' && 
                         selectedApproval.status !== 'pending-clarification' &&
                         selectedApproval.status !== 'for_clarification' &&
                         selectedApproval.status !== 'pending_clarification' && (
                          <Button
                            variant="outline"
                            onClick={() => setShowClarificationModal(true)}
                            disabled={actionLoading !== null}
                            className="border-2 border-sky-200 text-sky-700 hover:bg-sky-50 hover:border-sky-300 px-8 py-3 text-base font-medium shadow-lg hover:shadow-xl transition-all duration-200"
                            size="lg"
                          >
