...
export async function GET() {
  try {
    ...
    const existingTemplate = await prisma.email_templates.findUnique({
      where: {
        template_key: 'notify-approver-added'
      }
    });
    ...
    const newTemplate = await prisma.email_templates.create({
      data: {
        template_key: 'notify-approver-added',
        title: 'Notify Approver - Added to Request',
        subject: 'IT HELPDESK: You have been added as an approver - Request #${Request_ID}',
        header_html: null,
        content_html: `<div style="padding: 32px; background-color: #f8fafc; font-family: Arial, sans-serif; line-height: 1.6;">
