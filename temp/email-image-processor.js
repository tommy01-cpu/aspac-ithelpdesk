 ...
 */
export async function processImagesForEmail(htmlContent, requestId) {
    if (!htmlContent) {
        return {
    ...
    try {
        ...
        while ((match = base64ImageRegex.exec(htmlContent)) !== null) {
            try {
                ...
                const newImgTag = fullMatch.replace(/src="data:image\/[a-zA-Z]*;base64,[^"]+"/, `src="${process.env.NEXTAUTH_URL || 'http://localhost:3000'}${publicUrl}"`);
 ...
 */
export async function cleanupOldRequestImages(maxAgeHours = 168) {
    try {
        const uploadsDir = path.join(process.cwd(), 'public', 'uploads', 'request-images');
        try {
            await fs.access(uploadsDir);
 ...
 */
export function stripImagesFromHtml(htmlContent) {
    if (!htmlContent)
        return htmlContent;
    // Replace image tags with placeholder text
    return htmlContent.replace(/<img[^>]*>/gi, '<div style="border: 1px solid #ccc; padding: 8px; margin: 4px 0; background: #f5f5f5; color: #666; font-size: 12px;">[Image attachment - view full request for images]</div>');
}
/**
